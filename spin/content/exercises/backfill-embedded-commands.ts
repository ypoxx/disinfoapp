...
export const backfillEmbeddedCommands: Exercise[] = [
  ...
  },
  {
    ...
    scenario: {
      de: 'Nachfass-E-Mail einer Fachkonferenz an eine bereits angemeldete Referentin. Die eigentliche Aufforderung steht — in Versalien hervorgehoben — mitten in einem Satz, der vordergründig nur den Ablauf des Konferenztags ankündigt: „Damit Ihr Slot am Vormittag reibungslos läuft, BESTÄTIGEN SIE VORAB IHRE TEILNAHME UND REICHEN IHRE FOLIEN EIN." Allein die Großschreibung hebt die Handlungsphrase heraus; der Satz selbst organisiert an der Oberfläche nur den Tagesablauf.',
      en: 'A follow-up email from a professional conference to an already registered speaker. The actual instruction stands — set in capitals — in the middle of a sentence that ostensibly only announces the run of the conference day: "So that your morning slot runs smoothly, CONFIRM YOUR ATTENDANCE IN ADVANCE AND SUBMIT YOUR SLIDES." The capitalisation alone lifts the action phrase out; on its surface the sentence merely arranges the day\'s schedule.',
    },
    options: [
      {
        de: 'eine Handlungsaufforderung („Teilnahme bestätigen, Folien einreichen") in einen Satz einbettet, der vordergründig nur den Tagesablauf ankündigt, und sie durch Versalien markiert (Eingebettete Befehle)',
        en: 'embedding an instruction ("confirm attendance, submit your slides") in a sentence that ostensibly only announces the day\'s schedule and marking it with capitals (embedded commands)',
      },
      {
    ...
    correctAnswers: [0],
    explanation: {
      de: 'Eingebettete Befehle: Der eigentliche Imperativ „Teilnahme bestätigen und Folien einreichen" ist in einen Satz gepackt, der vordergründig nur den Tagesablauf organisiert — und im geschriebenen Kanal übernimmt die Versalienschreibung die Rolle, die im Gespräch Betonung und Pause spielen: Sie hebt die Handlungsphrase heraus, ohne sie als Aufforderung auszuweisen. Genau diese Tarnung ist das Kennzeichen; die empirische Steuerungswirkung selbst ist schwach belegt. Legitim einsetzen: Setz deine Handlungsaufforderung sichtbar als eigenen Satz und begründe sie — „Bitte reichen Sie Ihre Folien bis Donnerstag ein, dann prüfen wir die Technik vorab." Sauberes Handwerk zeigt die Bitte, statt sie zu verstecken. Erkennen und kontern: Lies den Satz einmal ohne die Hervorhebung und frag dich, was übrig bleibt — steckt darin eine Anweisung, die der Hauptsatz gar nicht ankündigt, ist der Hebel entlarvt; benenne ihn dann ausdrücklich als das, was er ist. Antwort 2 (But-You-Are-Free) beschreibt einen echten Nachbarn — das ausdrückliche Betonen der Wahlfreiheit —, doch der Text enthält gar keine Freiheitsformel; er markiert eine Handlung, statt die Autonomie der Empfängerin zu bekräftigen. Pacing und Leading setzte voraus, dass zuvor mehrere unstrittige Feststellungen Zustimmung sammeln, an die sich die Führung anhängt — davon steht hier kein Wort. Framing würde denselben Schritt in eine wertende Perspektive rücken, sodass die Teilnahme als selbstverständlich kluge Wahl erschiene — hier wird nicht bewertet, sondern eine markierte Anweisung untergeschoben.',
      en: 'Embedded commands: the actual imperative "confirm attendance and submit your slides" is packed into a sentence that ostensibly only arranges the day\'s schedule — and in the written channel the capitalisation takes on the role that stress and a pause play in speech: it lifts the action phrase out without labelling it as a request. That disguise is the hallmark; the steering effect itself is weakly evidenced. Used legitimately: put your call to action visibly as its own sentence and give a reason — "Please submit your slides by Thursday, so we can check the technical setup in advance." Clean craft shows the request rather than hiding it. To spot and counter it: read the sentence once without the emphasis and ask what remains — if it contains an instruction the main clause never announced, the lever is exposed; then name it plainly for what it is. Answer 2 (but-you-are-free) describes a genuine neighbour — expressly stressing freedom of choice — but the text contains no freedom formula at all; it marks an action rather than affirming the recipient\'s autonomy. Pacing and leading would presuppose that several unarguable statements first gather agreement for the lead to attach to — not a word of that here. Framing would place the same step in an evaluative light, so that attending looks like the self-evidently smart choice — here nothing is being evaluated; a marked instruction is slipped in instead.',
    },
    points: 10,
    primaryTechniqueId: 'embedded_commands',
