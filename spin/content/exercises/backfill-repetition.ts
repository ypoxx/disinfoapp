...
export const backfillRepetition: Exercise[] = [
  ...
  },
  {
    ...
    options: [
      ...
      {
        de: 'die zentrale Steuerung der Kampagne zu verschleiern, sodass die vielen Stimmen wie eine spontane Graswurzelbewegung wirken (Astroturfing)',
        en: 'disguise the campaign\'s central coordination so that the many voices look like a spontaneous grassroots movement (astroturfing)',
      },
      {
        de: 'die Öffentlichkeit mit einer hohen Zahl ständig wechselnder, teils widersprüchlicher Behauptungen zu überfluten, bis die Prüfung nicht mehr hinterherkommt (Firehose of Falsehood)',
        en: 'flood the public with a high volume of constantly shifting, partly contradictory claims until fact-checking can no longer keep up (firehose of falsehood)',
      },
    ],
    correctAnswers: [0],
    explanation: {
      de: 'Wiederholung: Das Kernprinzip ist die schiere Frequenz — dieselbe Aussage, wortgleich und aus vielen Mündern, bis sie unausweichlich wird. Nicht ein Argument trägt sie, sondern ihre koordinierte Allgegenwart. Sauber genutzt: Halte deine belegte Kernbotschaft in Kampagne und Anhörung diszipliniert durch, statt sie in jeder Runde neu zu erfinden — abgestimmte, einheitliche Advocacy ist im Verbandsgeschäft völlig sauber, solange die geteilte Aussage belegt ist und du jeden Absender offen als Interessenvertreter erkennbar lässt; problematisch wird es erst, wenn die Formel den fehlenden Beleg ersetzt. Erkennen und kontern: Frag dich, ob dich der Satz überzeugt, weil du ihn geprüft hast — oder nur, weil er dir aus jeder Richtung gleichlautend begegnet ist; und dann verlang genau dort die Substanz, die die Wiederholung ersetzt („Woran messen Sie das? Zeigen Sie die Zahlen."), statt den Satz weiterzureichen. Social Proof (Option 2) beruft sich auf sichtbare Mehrheiten — hier zählt nicht, dass viele zustimmen, sondern dass ein und derselbe Satz vielfach wiederholt wird. Astroturfing (Option 3) würde die zentrale Steuerung verstecken und die Stimmen als spontane Graswurzelbewegung ausgeben — der Verband tritt aber offen als Absender auf, verschleiert nichts; er wiederholt bloß. Firehose of Falsehood (Option 4) flutet mit vielen wechselnden, teils widersprüchlichen Behauptungen — das Gegenteil der einen, unveränderten Zeile, die hier durch Konstanz statt durch Masse wirkt.',
      en: 'Repetition: the core principle is sheer frequency — the same statement, word for word and from many mouths, until it becomes unavoidable. It is carried not by an argument but by its coordinated omnipresence. Used cleanly: hold your evidenced core message with discipline across a campaign or a hearing instead of reinventing it every round — aligned, consistent advocacy is entirely clean in association work, as long as the shared statement is evidenced and you keep every sender openly identifiable as an interest group; it turns problematic only when the formula stands in for the missing evidence. To spot and counter it: ask whether the sentence convinces you because you have checked it — or only because it has reached you from every direction in identical wording; then demand, right there, the substance the repetition is replacing ("By what measure? Show the figures.") instead of passing the sentence on. Social proof (option 2) invokes visible majorities — here what counts is not that many agree, but that one and the same sentence is repeated many times over. Astroturfing (option 3) would hide the central coordination and pass the voices off as a spontaneous grassroots movement — but the association appears openly as the sender and conceals nothing; it merely repeats. Firehose of falsehood (option 4) floods with many shifting, partly contradictory claims — the opposite of the single, unchanging line that works here through constancy rather than volume.',
    },
    points: 10,
    primaryTechniqueId: 'repetition',
    relatedTechniques: ['repetition', 'social_proof', 'astroturfing_sockpuppets', 'firehose_of_falsehood'],
  },
];
