...
export const w4AutomationBiasOverreliance: Exercise[] = [
  ...
  },
  {
    ...
    explanation: {
      ...
      en: 'The confidence score is a system output with an unknown error rate for this specific case — turning it into the basis for a decision sight unseen is the mechanism of automation bias: the tool gets the final word before the obvious human check has happened (commission error). Used legitimately: verification tools speed up triage — you use them professionally by treating the score as one signal alongside independent lines of verification, with a pre-agreed threshold above which a second instance is mandatory. To recognise and counter it: before a tool output swings a crisis decision, secure the fastest independent check — here, the comparison with the plant and a forensic second opinion — and only then issue a line. The apology option looks plausible because 89% sounds like certainty and speed matters in a crisis — but it shifts the full weight of the decision onto a number whose error rates and training limits you do not know. The second run sounds diligent yet verifies nothing independent: the same model given the same material returns the same bias. Dropping the score on principle overcorrects — the tool provides a useful signal, just not one that replaces human verification; with modern imagery, the naked eye is the weakest reviewer of all.',
    },
    points: 10,
    primaryTechniqueId: 'automation_bias_overreliance',
