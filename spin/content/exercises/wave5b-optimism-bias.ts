...
export const wave5bOptimismBias: Exercise[] = [
  {
    ...
    scenario: {
      de: 'Auswertung der Projektrisiken in einer Bauträger-Gesellschaft: Die Projektcontrollerin zeigt, dass jedes dritte vergleichbare Vorhaben dieser Größe die Bauzeit um mehr als ein halbes Jahr reißt, und rät zu einem Zeitpuffer im Terminplan. Die Projektleiterin winkt ab: „Diese Quote kenne ich, aber unser eingespieltes Team hat das immer im Griff — uns trifft das nicht." Sie plant weiter ohne Puffer.',
      en: 'Project-risk review at a property developer: the project controller shows that one in three comparable schemes of this size overruns its build time by more than half a year, and advises a schedule buffer. The project lead waves it off: "I know that rate, but our well-drilled team has always kept this in hand — it won\'t hit us." She plans on without a buffer.',
    },
    options: [
    ...
    correctAnswers: [0],
    explanation: {
      de: 'Mechanismus: Die Projektleiterin akzeptiert die Basisrate für „die anderen", nimmt sich selbst aber aus — sie schätzt das eigene Risiko systematisch niedriger ein als das vergleichbarer Vorhaben („uns trifft das nicht"). Das ist der Kern des Optimismus-Bias: unrealistischer Optimismus über das eigene Risiko. So nutzt du gesunden Optimismus professionell: Zuversicht hält ein Team handlungsfähig — dosiert und an belegte Fortschritte gekoppelt ist sie in der Kommunikation legitim. Erkennen und kontern: Sobald jemand eine Statistik für andere gelten lässt, aber nicht für die eigene Lage, führ die Basisrate auf den konkreten Fall zurück und verlang ein Downside-Szenario mit Puffer, statt nur den Best Case zu planen. Die Verfügbarkeitsheuristik läge vor, wenn sie nach der Erinnerung an konkrete Fälle urteilte („ich habe noch nie eine Bauzeit reißen sehen, also unwahrscheinlich") — sie stützt sich hier aber nicht auf fehlende Erinnerung, sondern nimmt sich pauschal von einer akzeptierten Quote aus. Der Bestätigungsfehler beschriebe, dass sie gezielt nur Belege für die Stärke des eigenen Teams sammelt und Gegenteiliges ausblendet; hier weist sie die Zahl nicht mit Gegenbelegen ab, sondern erklärt sich schlicht zur Ausnahme. Die Repräsentativitätsheuristik läge vor, wenn sie das Risiko allein am Stereotyp „so sieht ein verzögerungsanfälliges Projekt aus" festmachte — der entscheidende Schritt ist hier aber, dass sie das eigene Risiko unter das vergleichbarer Vorhaben setzt, nicht die Ähnlichkeit zu einem Prototyp.',
      en: 'Mechanism: the project lead accepts the base rate for "the others" but exempts herself — she systematically judges her own risk lower than that of comparable schemes ("it won\'t hit us"). That is the heart of optimism bias: unrealistic optimism about one\'s own risk. Using healthy optimism professionally: confidence keeps a team able to act — dosed and tied to demonstrated progress, it is legitimate in communication. Recognising and countering it: the moment someone lets a statistic apply to others but not to their own situation, bring the base rate back to the concrete case and require a downside scenario with a buffer instead of planning only the best case. The availability heuristic would apply if she judged by the ease of recalling concrete cases ("I have never seen a build time overrun, so it is unlikely") — but here she does not lean on missing recall; she exempts herself wholesale from a rate she accepts. Confirmation bias would describe her gathering only evidence of her own team\'s strength and screening out the opposite; here she does not rebut the figure with counter-evidence, she simply declares herself the exception. The representativeness heuristic would apply if she pinned the risk solely on the stereotype "this is what a delay-prone project looks like" — but the decisive move here is that she sets her own risk below that of comparable schemes, not resemblance to a prototype.',
    },
    points: 10,
  ...
  },
  {
    ...
    scenario: {
      de: 'Mitarbeiter-Sprechstunde zur Berufsunfähigkeitsvorsorge bei einem Versicherer: Die Beraterin zeigt einem Angestellten die Ausfallquote seiner Berufsgruppe und rät zur Zusatzpolice. Er lehnt ab: „Solche Fälle treffen Kollegen mit Vorerkrankungen oder Raubbau am Körper. Ich bin fit und lebe vorsichtig — mich erwischt das nicht", und unterschreibt nur die Basisabsicherung.',
      en: 'Employee advice session on disability cover run by an insurer: the adviser shows an employee the incidence rate for his occupational group and recommends a top-up policy. He declines: "Cases like that hit colleagues with pre-existing conditions or who wear their bodies out. I\'m fit and I live carefully — it won\'t catch me," and signs only the basic cover.',
    },
    options: [
      { de: 'Optimismus-Bias', en: 'Optimism bias' },
      { de: 'Bestätigungsfehler', en: 'Confirmation bias' },
