...
export const wave5bStereotyping: Exercise[] = [
  {
    ...
    scenario: {
      de: 'Strategie-Meeting zur Neuausrichtung des Vertriebskanals: Die Marktforschungsleiterin präsentiert der Geschäftsführung ein Kundensegment: „Unsere Kundschaft aus dem ländlichen Raum ist durch die Bank preisgetrieben und wenig markentreu — das ist bei denen einfach so. Prämienprogramme und Servicepakete können wir uns bei der Gruppe sparen, die kaufen ohnehin nur das jeweils Billigste." Zahlen zum tatsächlichen Kaufverhalten legt sie nicht vor.',
      en: 'Strategy meeting on realigning the sales channel: the market-research lead presents a customer segment to the executive board: "Our rural customers are price-driven and barely brand-loyal across the board — that is just how they are. We can save ourselves any loyalty programme or service package for that group, they only ever buy whatever is cheapest." She presents no figures on actual purchasing behaviour.',
    },
    options: [
  ...
  },
  {
    ...
    scenario: {
      de: 'Kreativ-Abnahme für eine Kampagne zum Studierendenkonto einer Direktbank, die du als Kommunikationsverantwortliche betreust: Der Zielgruppen-Absatz im Agentur-Briefing lautet: „Studierende sind chronisch pleite, sprunghaft und nur über Rabatte zu ködern — Ansprache jung, laut, ausschließlich mobil." Der Absatz nennt keine Segmentdaten, sondern ein durchgängiges Klischee. Die Agentur wartet auf dein Okay.',
      en: 'Creative sign-off for a campaign around a student current account at a direct bank that you handle as communications lead: the audience paragraph in the agency brief reads: "Students are chronically broke, fickle and only hooked by discounts — messaging young, loud, mobile-only." The paragraph cites no segment data, only a blanket cliché. The agency is waiting for your go-ahead.',
    },
    options: [
      {
      ...
      {
        de: 'Ich ersetze das pauschale Klischee durch belegte Segmentdaten: Ich lasse die Zielgruppe nach tatsächlichem Finanzverhalten aufschlüsseln, benenne Untergruppen statt „die Studierenden" und schlage einen Ansprache-Test vor.',
        en: 'I replace the blanket cliché with evidenced segment data: I have the audience broken down by actual financial behaviour, name sub-groups instead of "students", and propose a test of the messaging.',
      },
      {
        de: 'Ich drehe das Klischee ins Positive und formuliere: „Studierende sind alle spontan, begeisterungsfähig und dankbar für jeden Rabatt" — so bleibt die Zielgruppe griffig und der Ton wohlwollend.',
        en: 'I flip the cliché into the positive and write: "Students are all spontaneous, enthusiastic and grateful for any discount" — that keeps the audience snappy and the tone benevolent.',
      },
    ],
    correctAnswers: [2],
    explanation: {
      de: 'Der Briefing-Absatz ist Stereotypisierung: Ein festes, vereinfachendes Gruppenbild wird als selbstverständlich vorausgesetzt und pauschal auf „die Studierenden" übertragen — die konkrete Zielgruppe verschwindet hinter der zugeschriebenen Eigenschaft. Der Hebel wirkt, weil ein vertrautes Etikett die Segmentanalyse ersetzt und der Kreation sofort eine Richtung gibt, die sich nur noch selbst bestätigt. Die beste Option nimmt dem Muster die Grundlage: Du behältst die legitime Absicht (eine klare Zielgruppe) bei, ersetzt aber die Pauschale durch belegte Segmentdaten, benennst Untergruppen und behandelst die Ansprache als testbare Hypothese. Legitim einsetzen: Zielgruppen zu bündeln und Muster zu benennen ist zulässig — der Unterschied ist, ob das Bild an Daten hängt und Streuung zulässt oder als feststehende Gruppeneigenschaft gesetzt wird. Erkennen und kontern: Verlang die Segmentierung nach echtem Verhalten und ein, zwei Gegenbeispiele aus der eigenen Datenbank — hält die Pauschale das nicht aus, ist sie ein Klischee. Den Absatz unverändert zu übernehmen adelt das Klischee zur Planungsgrundlage. Den Absatz ganz zu streichen wirft mit der Pauschale auch die nötige, datenbasierte Zielgruppenschärfe weg — Freiheit ersetzt keine Analyse. Das Klischee ins Positive zu drehen bleibt Stereotypisierung: „alle spontan und dankbar für Rabatte" ist dasselbe rigide Gruppenbild, nur mit umgekehrtem Vorzeichen.',
      en: 'The brief paragraph is stereotyping: a fixed, oversimplified image of a group is taken for granted and applied wholesale to "students" — the actual audience disappears behind the ascribed trait. The lever works because a familiar label replaces segment analysis and hands the creative team a direction that then only confirms itself. The best option removes that foundation: you keep the legitimate intent (a clear audience) but replace the blanket claim with evidenced segment data, name sub-groups, and treat the messaging as a testable hypothesis. Used legitimately: bundling audiences and naming patterns is fair — the difference is whether the image is anchored in data and allows for variation, or is set as a settled group trait. To spot and counter it: ask for segmentation by actual behaviour and one or two counter-examples from your own database — if the blanket claim cannot survive that, it is a cliché. Keeping the paragraph unchanged elevates the cliché into the basis for planning. Deleting the paragraph entirely throws out the necessary, data-based audience focus along with the blanket claim — freedom is no substitute for analysis. Flipping the cliché into the positive is still stereotyping: "all spontaneous and grateful for discounts" is the same rigid group image, merely with the sign reversed.',
    },
    points: 10,
    primaryTechniqueId: 'stereotyping',
