...
export const wave5cDivideAndRule: Exercise[] = [
  ...
  },
  {
    ...
    explanation: {
      ...
      en: 'The pattern is Divide and Rule: the party the joint bonus demand is aimed at keeps sales and service apart through separate channels carrying opposite messages, so their coalition collapses. The best option removes the technique\'s foundation — the missing direct exchange: you bring both sides together and put the shared starting point and the facts on the table, so the contradictory narratives no longer hold. Used legitimately: separate talks with individual stakeholders are normal — the difference is whether real interests are named or enemy images between allies are invented. To spot and counter it: the moment two groups with a common cause suddenly talk about each other instead of to each other, restore the direct channel and let them compare the claims. Backing one side first in a one-on-one deepens exactly the split you want to defuse. Staying out of it hands the field to the separate handling — the pattern keeps working. Publicly exposing the division head looks decisive but merely swaps one conflict for another and turns her into a scapegoat, instead of reuniting the two teams.',
    },
    points: 10,
    primaryTechniqueId: 'divide_and_rule',
