...
export const wave5cFirehoseOfFalsehood: Exercise[] = [
  ...
  },
  {
    ...
    correctAnswers: [0],
    explanation: {
      de: 'Firehose of Falsehood: viele, einander widersprechende Versionen desselben Ereignisses, gleichzeitig über mehrere Kanäle und in hohem Tempo — nicht um eine Version zu belegen, sondern um zu überfordern und den Eindruck zu erzeugen, gesicherte Wahrheit sei ohnehin nicht mehr feststellbar. Der Hebel ist die Überlastung: Prüfen kostet mehr Zeit als Behaupten, also weicht das Publikum auf „man weiß es nicht" aus. Legitim wäre konsistente Mehrkanal-Kommunikation mit einer verifizierten Kernbotschaft; zur Manipulation wird es durch die bewusst widersprüchliche Masse aus Falschversionen. Daran erkennst du es: viele offen widersprüchliche Versionen, gleichzeitig über viele Kanäle, in einem Tempo, das Richtigstellungen abhängt — dann setze eine einzige Faktenbasis und benenne das Muster, statt jeder Version hinterherzujagen. Coordinated Inauthentic Amplification spielt synchron dieselbe, identische Botschaft aus, um Schein-Konsens zu erzeugen — hier widersprechen sich die Versionen dagegen offen; nicht Einigkeit, sondern Verwirrung ist das Ziel. Gish Gallop meint eine einzelne Person, die in einer Wortmeldung viele Behauptungen aneinanderreiht; hier ist es keine einzelne Stimme in einer Debatte, sondern eine verteilte Flut über viele Kanäle und Absender. Conspiracy Theory Narratives bieten eine in sich geschlossene Gegenerzählung an; die Firehose braucht gerade keine Kohärenz — die Widersprüche sind Teil des Effekts.',
      en: 'Firehose of Falsehood: many contradictory versions of the same event, running simultaneously across several channels at high speed — not to substantiate any one version but to overwhelm and leave the impression that reliable truth can no longer be established. The lever is overload: checking costs more time than asserting, so the audience falls back on "who can say". Legitimate would be consistent multi-channel communication with one verified core message; it becomes manipulation through the deliberately contradictory mass of false versions. To spot it: many openly contradictory versions, simultaneously across many channels, at a pace that outruns corrections — then set one factual baseline and name the pattern rather than chasing each version. Coordinated inauthentic amplification pushes the same, identical message in sync to fake consensus — here the versions openly contradict one another; the goal is confusion, not agreement. Gish gallop means a single person stringing many claims together in one turn; here it is not one voice in a debate but a distributed flood across many channels and senders. Conspiracy theory narratives offer a self-contained counter-story; the firehose deliberately needs no coherence — the contradictions are part of the effect.',
    },
    points: 10,
    primaryTechniqueId: 'firehose_of_falsehood',
    relatedTechniques: ['firehose_of_falsehood', 'gish_gallop', 'conspiracy_theory_narratives'],
