...
export const wave5cForcedAction: Exercise[] = [
  ...
  },
  {
    ...
    correctAnswers: [0],
    explanation: {
      de: 'Forced Action: Zwischen dem Journalisten und seinem Ziel — dem vollständigen Text — steht eine sachfremde Pflichthandlung, die App-Installation, und einen Weg daran vorbei gibt es nicht. Der Zwang liegt offen; das Muster lebt nicht vom Verstecken, sondern davon, dass es kein Vorbei am Tor gibt. Legitim einsetzen: Eine App bewerben ist erlaubt — biete sie als Zusatzangebot an, aber halte den vollständigen Text im Browser zugänglich; gerade für Presseinhalte ist ein installationsfreier, direkt im Browser lesbarer Weg professioneller Standard — damit Journalist:innen den Text ohne Hürde übernehmen können. Erkennen und kontern: Prüf, ob es das Ziel auch ohne die verlangte Zusatzhandlung gibt — fehlt der Vorbei-Weg (hier: Weiterlesen im Browser), ist es Forced Action; für eigene Kanäle heißt das, Presse- und Kerninhalte niemals hinter eine App oder Registrierung zu zwingen. Sludge liegt nahe, weil das App-Overlay bremst — Sludge meint aber schädliche Reibung, die vom Ziel abbringen soll; hier ist es kein zäher Weg, sondern eine harte Vorbedingung ohne Alternative. Nagging wirkt plausibel, weil App-Installations-Einblendungen oft genau so auftauchen — Nagging meint aber die wiederholte, wegklickbare Aufforderung, die dich drängt, ohne dich zu blockieren; hier ist das Overlay keine Bitte, die du abweisen kannst, sondern eine harte Schranke ohne „im Browser weiterlesen". Genau das kippt die dranghafte Aufforderung in eine erzwungene Handlung.',
      en: 'Forced Action: between the journalist and his goal — the full text — sits an unrelated mandatory action, installing the app, and there is no way around it. The coercion is out in the open; the pattern lives not on concealment but on there being no route past the gate. Used legitimately: promoting an app is fine — offer it as an add-on, but keep the full text accessible in the browser; for press content in particular, an install-free route that reads straight in the browser is professional standard — so journalists can pick up the text without a hurdle. To spot and counter it: check whether the goal is reachable without the extra required action — if the way past is missing (here: reading on in the browser), it is Forced Action; for your own channels that means never forcing press and core content behind an app or registration. Sludge looks close because the app overlay slows you down — but sludge means harmful friction meant to wear you off the goal; here it is not a sluggish path but a hard precondition with no alternative. Nagging looks plausible because app-install prompts often show up exactly like this — but nagging means the repeated, dismissible prompt that pushes you without blocking you; here the overlay is not a request you can wave away but a hard barrier with no "read on in the browser". That is exactly what tips a naggy prompt over into a forced action.',
    },
    points: 10,
    primaryTechniqueId: 'forced_action',
