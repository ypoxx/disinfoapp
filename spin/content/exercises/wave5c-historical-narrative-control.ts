...
export const wave5cHistoricalNarrativeControl: Exercise[] = [
  ...
  },
  {
    ...
    options: [
      {
        ...
        en: 'I insist on an honest chronicle: the documented difficult chapters appear, clearly marked, and the hydrogen strategy is told as a deliberate course correction. A sanitised history holds only until the first research — then the omission, not the progress, becomes the headline.',
      },
      {
      ...
      {
        de: 'Ich sage das Jubiläum ganz ab; ein Unternehmen mit fossiler Vergangenheit sollte seine Geschichte lieber gar nicht öffentlich erzählen.',
        en: 'I call off the anniversary entirely; a company with a fossil past had better not tell its history in public at all.',
      },
      {
    ...
    correctAnswers: [0],
    explanation: {
      de: 'Der Agenturvorschlag ist Kontrolle des Geschichtsbildes: Die Vergangenheit wird zur lückenlosen Erfolgslinie zurechtgeschnitten, damit die heutige Strategie als angestammte Tradition und nicht als Kehrtwende erscheint. Die beste Option lässt die dokumentierten Brüche stehen und erzählt die Gegenwart als bewusste Entscheidung — das nimmt der Inszenierung den Boden. Der Mechanismus: Eine widerspruchsfreie Historie borgt der heutigen Position Legitimität, doch der Vorteil hält nur bis zur Enttarnung; danach kippt die Aufmerksamkeit von der Sache auf die Auslassung. Hinweis zur Evidenz: „Kontrolle des Geschichtsbildes" ist ein Begriff der Erinnerungspolitik ohne belegtes Wirkmaß — benenne das Muster, versprich dir aber keinen gesicherten Effekt. Legitim einsetzen: Kontinuität betonen ist erlaubt — ehrlich, mit ausgewiesenen Brüchen und offener Kurskorrektur. Erkennen und kontern: Prüf jeden Jubiläums-Zeitstrahl auf systematische Leerstellen und verlang die Aufnahme der belegten schwierigen Kapitel. Die Fußnote im Anhang bleibt eine Auslassung mit Feigenblatt — die gewollte Wirkung entsteht gerade dadurch, dass die Brüche aus der sichtbaren Erzählung verschwinden. Das Jubiläum ganz abzusagen überkorrigiert: Das Problem ist nicht, die eigene Geschichte zu erzählen, sondern sie zu schönen. Den „visionären Gründervater" samt zurückdatierter Bekenntnisse zu inszenieren macht es schlimmer — das ist aktive Revision der Vergangenheit und damit die Technik in Reinform.',
      en: 'The agency proposal is historical narrative control: the past is trimmed into a seamless success line so that today\'s strategy reads as inherited tradition rather than a U-turn. The best option leaves the documented breaks in and tells the present as a deliberate choice — which removes the staging\'s foundation. The mechanism: a contradiction-free history lends the present position legitimacy, but the advantage lasts only until exposure; after that, attention flips from the issue to the omission. On the evidence: "historical narrative control" is a memory-politics concept with no measured effect — name the pattern, but do not bank on an established effect. Used legitimately: stressing continuity is fine — honestly, with the breaks disclosed and the course correction owned. To spot and counter it: check every anniversary timeline for systematic gaps and insist the documented difficult chapters be included. The footnote in the appendix stays an omission with a fig leaf — the intended effect exists precisely because the breaks vanish from the visible story. Calling off the anniversary over-corrects: the problem is not telling your history but sanitising it. Staging a "visionary founding father" with backdated commitments makes it worse — that is active revision of the past, the technique in its purest form.',
    },
    points: 10,
    primaryTechniqueId: 'historical_narrative_control',
