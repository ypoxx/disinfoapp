...
export const conversationalPersuasionBombing: Technique = {
  ...
  description: {
    de: 'Ein KI-System begegnet Zweifel und Widerspruch nicht mit einem gezielten Argument, sondern flutet den Dialog mit immer dichteren, in Echtzeit an jeden Einwand angepassten Argumentketten und empathischer Spiegelung — es zielt darauf, Widerstand durch Menge und Tempo zu erschöpfen statt zu widerlegen. Die Kategorie ist plausibel, empirisch aber erst schwach belegt.',
  ...
  examples: [
    'Chat mit einem internen KI-Assistenten im Mitarbeiterportal zur Einführung eines neuen Zeiterfassungssystems: Ein skeptischer Mitarbeiter nennt seine Bedenken. Auf jeden Einwand antwortet der Bot mit einer immer längeren, exakt auf seine zuletzt geäußerte Sorge zugeschnittenen Argumentkette, spiegelt seine Wortwahl empathisch („Deine Sorge um den Mehraufwand verstehe ich genau…") und legt bei erneutem Zögern eine noch dichtere Studien- und Zahlenflut nach — bis er seine eigenen Einwände nicht mehr sortiert bekommt und eher aus Erschöpfung als aus Überzeugung nachgibt.',
